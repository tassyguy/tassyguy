### Hi there, I'm Simon! 👋
<p align="center">
    <img alt="contribution snake" width="85%" src="https://raw.githubusercontent.com/1amSimp1e/1amSimp1e/544749532955a2438af7a7934a17731aa0ea1b43/github-contribution-grid-snake.svg" />
</p>

## About Me

- 🧔🏻‍♂️ Pronouns: He/Him/His and They/Them/Theirs
- 🍑 Currently in Atlanta, Georgia
- 🌏 Originally from Melbourne, Victoria.
- 💾 Started coding through old Pokémon games and game consoles.
- 🐧 Linux stan, but fluent in macOS and Windows environments too.

## Tech Stack

- 🚀 Over 8 years of JavaScript development experience.
- ☕ 4+ years professional experience with both Java and C#, incl. Java Spring and .NET.
- ☁️ 4 years of Azure experience and 3 years with AWS.
- ⚛️ In LTR with React for 5 years (because it's a library and lightweight).
- 🧰 3 year TypeScript fanboy (for the added type safety).
- ⚡ Can also work in ES6, Angular, and Vue-based setups.

## Personal Tidbits

- 🎮 Gaymer, kickballer, traveler and aspiring streamer (thanks iCarly).
- 🐈 Sassy orange tabby named Tangy.
- 🧪 Main in Overwatch 2: Moira, because I like to win.
- 🕹️ Super Smash Bros. main: Lucas in Project M and Ultimate.
- 🦠 Favorite Pokémon: Porygon-Z, the glitched computer program Pokémon.

## Get in Touch

- 📫 Connect with me on [LinkedIn](https://www.linkedin.com/in/simonxphillips).
- 🌐 Visit my [website](https://simonxphillips.com).
- 📜 Download my [resume](https://drive.google.com/file/d/10zoCEWRpc8XblT5ZZE10HWRTkMMMTuu0/view?usp=sharing),
- 📧 Drop me an email at [simonxphillips@gmail.com](simonxphillips@gmail.com).
<!-- 🐦 Tweet me [@YourTwitterHandle](https://twitter.com/YourTwitterHandle). -->
<!--
**tassyguy/tassyguy** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
Here are some ideas to get you started:
-->
